...
def get_gateway_address():
    """Dynamically extract Access Point IP from URL parameters, headers, or incoming remote address."""
    for key in ['gw_address', 'gw_ip', 'gwaddress', 'router_ip']:
        val = get_param(key)
        if val:
            return val.strip()
            
    # Check proxy header if hosted behind Cloudflare, Nginx, or Heroku
    forwarded_for = request.headers.get('X-Forwarded-For')
    if forwarded_for:
        return forwarded_for.split(',')[0].strip()
        
    return request.remote_addr or DEFAULT_GW_ADDRESS
...
def process_login():
    """Validates voucher and uses an explicit HTML auto-submitting form to bypass HTTPS->HTTP security blocks."""
    ...
    # 3. Target local router auth URL dynamically
    auth_action_url = f"http://{gw_address}:{gw_port}/wifidog/auth"

    # 4. Render HTML form auto-submission page
    return f"""
    <!DOCTYPE html>
    <html lang="sw">
    <head>
    ...
    <body>
        <div class="card">
            ...
            <form id="authForm" action="{auth_action_url}" method="GET">
                <input type="hidden" name="token" value="{token}">
                <button type="submit" class="btn">Bonyeza Hapa Kama Haujaunganishwa</button>
            </form>
        </div>
