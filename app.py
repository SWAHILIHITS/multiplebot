...
def get_client_mac():
    """Detect client MAC address across common gateway query keys."""
...
def captive_login_page():
    """Renders voucher entry page or auto-logs in if device session is already active."""
    mac = get_client_mac()
    ...
    # Automatically bypass login if this MAC address has an active, unexpired session
...
def process_login():
    """Validates voucher and issues a direct HTTP 303 Redirect to local AP auth endpoint."""
    ...
    # 4. Direct HTTP 303 Redirect (Forces clean GET request in Captive Portal WebView)
    return redirect(auth_action_url, code=303)
...
def wifidog_auth_check():
    ...
    Must strictly return plain-text 'Auth: 1' or 'Auth: 0'.
    """
    try:
        ...
        # Check MAC Address (Periodic ping phase)
        if mac:
