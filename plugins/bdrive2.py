...
# Global tracking for single-request execution and cancellation
...
async def process_and_upload_video(video_obj, token, id2, c, progress_msg, user_id):
    ...
    try:
        ...
        # Immediate cancellation check during FFmpeg execution
        ...
        # MoviePy for Thumbnail and Duration
...
async def on_sync(client, message):
    ...
    # Signal cancellation of existing task
    if user_id in active_syncs:
        active_syncs[user_id] = "cancel"
        await message.reply("🛑 **Stopping previous sync to start new process...**")
        await asyncio.sleep(5) 

    active_syncs[user_id] = "running"
    status_msg = await message.reply("⏳ **Locking process...**")
    ...
    async with sync_lock:
        try:
            while active_syncs.get(user_id) == "running":
                ...
                if report == "Cancelled":
                    await status_msg.edit("✅ **Old process killed and replaced.**")
                    break
