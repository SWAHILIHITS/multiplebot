...
async def validate_id(service, file_id, name_type):
    try:
        ...
        return False, f"❌ **{name_type} Error:** {e.resp.status}"
    ...
    for n in range(max_retries):
        try:
            ...
            if status in [403, 429] and reason in ["rateLimitExceeded", "userRateLimitExceeded"]:
                ...
                # Added sleep for exponential backoff
...
def get_folder_contents(service, folder_id):
    ...
    page_token = None
    while True:
        # Includes shortcutDetails to resolve original files
    ...
    for name, item in source_items.items():
        ...
        if total > 0 and total % 10 == 0:
            ...
            try:
                ...
                pass
            # Slight sleep to avoid Telegram flood limits
...
async def addfilesondrive(client, message):
    ...
    msg_check = await message.reply("🔍 **Validating Links...**")
    ...
    stats = {'copied': 0, 'skipped': 0, 'total_bytes': 0}
    start_time = time.time()
    
    await recursive_copy(service, source_id, dest_id, client, user_id, stats, msg_check, start_time)
    
    final_status = "✅ Imekamilika kikamilifu!" if ACTIVE_TASKS.get(user_id) != "CANCELLED" else "🛑 Imesitishwa na mtumiaji!"
